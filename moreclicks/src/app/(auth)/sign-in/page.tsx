          ...
          <CardContent className="space-y-6">
              ...
              onClick={async () => {
                try {
                  ...
                  if (result?.error) {
                    ...
                    window.location.href = `/sign-in?error=${result.error}`
                  }
                } catch (error: any) {
                  console.error('Sign in error:', error)
                  // Redirect to error page
                  window.location.href = `/sign-in?error=${encodeURIComponent(error?.message || 'UnknownError')}`
                }
              }}
            >
            ...
            <div className="grid grid-cols-3 gap-4 pt-4 border-t border-purple-200/50">
              <div className="text-center">
                <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 mb-2">
                  <Shield className="h-6 w-6 text-white" />
                </div>
                <p className="text-xs font-semibold text-purple-700">Secure</p>
              </div>
              <div className="text-center">
                <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-gradient-to-br from-purple-400 to-purple-600 mb-2">
                  <Zap className="h-6 w-6 text-white" />
                </div>
                <p className="text-xs font-semibold text-purple-700">Fast</p>
              </div>
              <div className="text-center">
                <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-gradient-to-br from-pink-400 to-pink-600 mb-2">
                  <TrendingUp className="h-6 w-6 text-white" />
                </div>
                <p className="text-xs font-semibold text-purple-700">Powerful</p>
              </div>
            </div>

            <p className="text-center text-sm text-purple-600 dark:text-purple-400 font-medium pt-2">
              Don't have an account?{' '}
              <Link href="/sign-up" className="text-purple-700 dark:text-purple-300 font-bold hover:text-purple-900 dark:hover:text-purple-200 hover:underline transition-colors">
                Sign up
              </Link>
            </p>
          </CardContent>
